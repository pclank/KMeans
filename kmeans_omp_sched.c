...
void createVectors2(void)
{
    #pragma omp parallel                    // Create Parallel Section
    {
        ...
        #pragma omp for private(i, j) schedule(ScheduleType) ordered    // Parallelize First For - Loop
        for (i = 0; i < N; i++)
        {
            #pragma omp simd
            for (j = 0; j < Nv; j++)
...
void calcDistance2(void)
{
    ...
    #pragma omp parallel for private(flag, i, j, k, distance) shared(classes, cluster) schedule(ScheduleType) ordered      // Parallelize First For-Loop
...
void calcCentroids2(void)
{
    ...
    #pragma omp parallel for private(cnt, i, j, k) shared(vector_num, centroids) schedule(ScheduleType) ordered     // Parallelize First For - Loop
